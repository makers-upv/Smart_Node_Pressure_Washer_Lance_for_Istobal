...
void  Screen :: logoIstobal() {
  img_stat = reader.drawBMP("/Pantalla_1.bmp", tft, 0, 0); //Shows the first initial screen (Istobal logo)
}

void  Screen :: welcome() {
  img_stat = reader.drawBMP("/Pantalla_2.bmp", tft, 0, 0); //Shows the first initial screen (Istobal logo)
}

void  Screen :: waveStart() {
  img_stat = reader.drawBMP("/Pantalla_3.bmp", tft, 0, 0); //Shows the first initial screen (Istobal logo)
}

void Screen :: redScreen() {
  tft.fillScreen(RED); //Fills the screen white so no residual images in the screen memory are seen
}

void Screen :: greyScreen() {
  tft.fillScreen(GREY); //Fills the screen white so no residual images in the screen memory are seen
}
